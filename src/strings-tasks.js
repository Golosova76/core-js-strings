...
function formatTime(minutes, seconds) {
  const formaMinutes = String(minutes).padStart(2, '0');
  const formaSeconds = String(seconds).padStart(2, '0');
  return `${formaMinutes}:${formaSeconds}`;
}
...
function isPalindrome(str) {
  const clearStr = str.replace(/[^A-Za-z]/g, '').toLowerCase();
  const countLength = clearStr.length;

  for (let i = 0; i < countLength / 2; i += 1) {
    if (clearStr[i] !== clearStr[countLength - i - 1]) {
      return false;
    }
  }
  return true;
...
function reverseWords(str) {
  ...
  const reverseString = str.split(' ').map((value) => {
...
function extractNameFromTemplate(value) {
  const nameTemplate = value.replace('Hello, ', '');
  const nameTemplateTwo = nameTemplate.replace('!', '');
  return nameTemplateTwo;
}
